...
RUN sudo yum -y install python3-devel CUnit-devel bind-utils vim htop git wget tar rpm-build rake doxygen\
...
# Install DPDK v20.11
RUN set -o pipefail && curl -sL https://static.dpdk.org/rel/dpdk-20.11.tar.xz | tar -C . -xJ
WORKDIR ${HOME}/dpdk-20.11
...
WORKDIR ${HOME}/dpdk-20.11/build
RUN sudo ninja install
RUN sudo find /usr/local/lib -name 'librte_*.a' -delete
RUN sudo ldconfig
WORKDIR ${HOME}
RUN sudo rm -rf dpdk-20.11
ENV PKG_CONFIG_PATH=$PKG_CONFIG_PATH:/usr/local/lib/pkgconfig

# Install SPDK v20.10
RUN set -o pipefail && curl -sL https://github.com/spdk/spdk/archive/v20.10.tar.gz | tar -C . -xz
WORKDIR ${HOME}/spdk-20.10
